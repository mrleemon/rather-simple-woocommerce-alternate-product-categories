...
class WooCommerce_Alternate_Product_Categories extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ($dropdown) {
			...
			$output .= '<option value="" ' . selected( $term, '', false ) . '>' . __( 'Select a category', 'woocommerce' ) . '</option>';
			...
			if ($termid->parent > 0) { 
				...
				foreach ($siblingcategories as $siblingcategory) { 
					if ($siblingcategory->term_id == $term ) {
						echo '<li class="cat-item cat-item-' . $siblingcategory->term_id . ' current-cat">';
					} else {
						echo '<li class="cat-item cat-item-' . $siblingcategory->term_id . '">';
					}
				...
				foreach ($subcategories as $subcategory) {
					echo '<li class="cat-item cat-item-' . $subcategory->term_id . '"><a href="' . get_term_link( $subcategory ) . '">' . $subcategory->name . '</a>';
					if ($count) {
